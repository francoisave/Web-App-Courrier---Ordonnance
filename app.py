...
if choix_principal == "Courrier":
    ...
    if categorie == "Courrier vierge":
        ...
        nom_salarie  = st.text_input("Concernant :", placeholder="Nom et Prénom du salarié (optionnel)", key="nom_salarie_vierge")
...
if can_generate:
    ...
    if st.button("📄 Générer le document Word", type="primary"):
        try:
            ...
            # Nom du salarié → balise {{ salarié }} du template (champ "Concernant :")
            salarie_val = ""
            if choix_principal == "Courrier":
                if categorie == "Courrier vierge":
                    salarie_val = st.session_state.get("nom_salarie_vierge", "")
                elif categorie == "Médecin traitant":
